...
test("1 locator fill + type + click", async ({ page }) => {
  ...
  await expect(page.locator('#output #email')).toHaveText("Email:vadim@test.com");
});
...
test("2.1 radiobutton check", async ({ page }) => {
  ...
  await expect(page.locator('.custom-control-label:has-text("Yes")')).toBeChecked();
});
...
test("7 keys press", async ({ page }) => {
  await page.goto("https://demoqa.com/text-box");
  await page.getByPlaceholder('Current Address').type('211400 Polotsk, Streletskaya 5');
  ...
  await expect(page.locator('#output #currentAddress')).toHaveText('Current Address :211400 Polotsk, Streletskaya 5');
  await expect(page.locator('#output #permanentAddress')).toHaveText('Permananet Address :211400 Polotsk, Streletskaya 5');
});
